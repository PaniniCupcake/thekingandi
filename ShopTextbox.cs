...
public class ShopTextbox : MonoBehaviour
{
    ...
    public List<Sprite> bigwQueue = new List<Sprite>();
    public List<Sprite> bigbQueue = new List<Sprite>();
    public AudioSource click;
    private GameData dat;
    public GameObject bpawn;
    public GameObject wpawn;
    public List<Sprite> wpawn_sprites;
    public List<Sprite> bpawn_sprites;
    public List<Sprite> wpawn_sprites_big;
    public List<Sprite> bpawn_sprites_big;
    ...
    bool assign_Lines()
    {
        ...
        if (string_index < startStrings.Count)
        {
                ...
                if (chars[j] == '#')//# must be followed with appropriate chars for everything to work
                {
                    ...
                    else if (chars[j] == 's')
                    {
                        ...
                        if (chars[j] == 'w')
                        {
                            j++;
                            asc = chars[j] - 48;
                            spriteQueue[check_count] = wpawn_sprites[asc];
                            bigwQueue[check_count] = wpawn_sprites_big[asc];
                            bigbQueue[check_count] = bpawn_sprites_big[0];
                        }
                        else
                        {
                            j++;
                            asc = chars[j] - 48;
                            spriteQueue[check_count] = bpawn_sprites[asc];
                            bigbQueue[check_count] = bpawn_sprites_big[asc];
                            bigwQueue[check_count] = wpawn_sprites_big[0];
                        }
                    }
                    j++;
                    if(chars[j] == '#')
            ...
            if (!check_cancelled)
            {
                ...
                {
                    print("Hyh?!?!?");
                    bpawn.GetComponent<SpriteRenderer>().sprite = bigbQueue[0];
                }
                if (bigwQueue[0] != null)
                {
                    print("Hyh?!?!?");
                    wpawn.GetComponent<SpriteRenderer>().sprite = bigwQueue[0];
                }
            }
            for (i = 0; i < 4; i++)
            {
            ...
            if (Input.GetAxis("Skip") > 0 || Input.GetAxis("Submit") > 0)
            {
                ...
                else
                {
                    ...
                    if (next_check[check_count] >= 0 && check_count < 4)
                    {
                        ...
                        if (bigbQueue[check_count] != null)
                        {
                            bpawn.GetComponent<SpriteRenderer>().sprite = bigbQueue[check_count];
                        }
                        if (bigwQueue[check_count] != null)
                        {
                            wpawn.GetComponent<SpriteRenderer>().sprite = bigwQueue[check_count];
                        }
                        check_count++;
                    }
    ...
    private void Close()
    {
        wpawn.GetComponent<SpriteRenderer>().sprite = wpawn_sprites_big[0];
        bpawn.GetComponent<SpriteRenderer>().sprite = bpawn_sprites_big[0];
        Destroy(letter_container);
        if (shop != null)
        {
            shop.dialogue = false;
            shop.cooldown = -1;
            shop.back_cooldown = -1;
        }
